  ...
  return (
    <motion.div variants={fadeInUp} className="space-y-6">
      <GlassPanel title="Mental Health Impact" description="COVID-19 psychological effects on UAE residents">
          ...
          <Card className="glass-card bg-glass-surface/50 border-glass-border">
            ...
            <CardContent className="space-y-4">
              <div className="text-center p-4 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
                ...
                <div className="text-sm text-platinum-500-400">Reported Negative Mental Health Impact</div>
                <div className="text-xs text-platinum-500-500 mt-1">Source: YouGov Survey (April 2022)</div>
              </div>

              <div className="space-y-2">
                <div className="text-sm font-medium text-platinum-500-300">Most Affected Groups</div>
                {mentalHealthImpact.affectedPopulations.map((pop, idx) => (
                  ...
                  >
                    <Users className="h-4 w-4 text-platinum-500-500" />
                    <div className="flex-1">
                      <div className="text-sm text-platinum-500-200">{pop.group}</div>
                      <div className="text-xs text-platinum-500-500">{pop.impact}</div>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          <Card className="glass-card bg-glass-surface/50 border-glass-border">
            ...
            <CardContent className="space-y-4">
              <BarChart
                ...
                height={180}
              />
              <div className="text-xs text-platinum-500-500 text-center">
        ...
        {/* UAE Response */}
        <div className="mt-6">
          <h4 className="text-sm font-medium text-platinum-500-300 mb-3">UAE Response</h4>
          <div className="grid gap-3 md:grid-cols-2">
            {mentalHealthImpact.uaeResponse.map((response, idx) => (
              ...
              >
                ...
                <div>
                  <div className="text-sm font-medium text-emerald-500">{response.initiative}</div>
                  <div className="text-xs text-platinum-500-400 mt-1">{response.detail}</div>
                </div>
              </motion.div>
            ))}
          </div>

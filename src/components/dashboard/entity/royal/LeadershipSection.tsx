    ...
    <div className="space-y-6">
      ...
      <motion.div {...fadeInUp}>
        <GlassPanel
          ...
          icon={<Calendar className="h-5 w-5 text-navy-500-600" />}
        >
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-platinum-200/50 dark:border-platinum-700/50">
                  <th className="text-left py-2 px-3 text-platinum-500-700 dark:text-platinum-500-300 font-semibold">Person</th>
                  <th className="text-left py-2 px-3 text-platinum-500-700 dark:text-platinum-500-300 font-semibold">Position</th>
                  <th className="text-left py-2 px-3 text-platinum-500-700 dark:text-platinum-500-300 font-semibold">Date</th>
                </tr>
              </thead>
              <tbody>
                {keyLeadershipAppointments.map((appt, i) => (
                  <tr key={i} className="border-b border-platinum-100/50 dark:border-platinum-800/50 hover:bg-platinum-50/50 dark:hover:bg-platinum-800/25 transition-colors">
                    <td className="py-2 px-3 font-semibold text-navy-500-900 dark:text-platinum-500-100">{appt.person}</td>
                    <td className="py-2 px-3 text-xs text-platinum-500-600 dark:text-platinum-500-400">{appt.position}</td>
                    <td className="py-2 px-3 text-xs text-platinum-500-500">{appt.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid gap-3">
            {majorInitiatives.map((init, i) => (
              <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-platinum-50/50 dark:bg-platinum-800/20 border border-platinum-200/30 dark:border-platinum-700/30">
                <div>
                  <p className="text-sm font-semibold text-navy-500-900 dark:text-platinum-500-100">{init.leader}</p>
                  <p className="text-xs text-platinum-500-600 dark:text-platinum-500-400">{init.initiative}</p>
                </div>
              ...
              .map((member) => (
                <div key={member.id} className="flex items-center gap-3 p-3 rounded-lg bg-platinum-50/50 dark:bg-platinum-800/20 border border-platinum-200/30 dark:border-platinum-700/30">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-navy-500-900 dark:text-platinum-500-100 truncate">{member.name}</p>
                    <p className="text-xs text-platinum-500-500 truncate">{member.role}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-sm font-bold text-gold-700">{member.uaeRelevance}/10</span>
                      <div className="w-12">
      ...
      <motion.div {...fadeInUp}>
        <GlassPanel
          ...
          icon={<Scale className="h-5 w-5 text-navy-500-600" />}
        >
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {sourceCredibility.map((src, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-platinum-50/50 dark:bg-platinum-800/20 border border-platinum-200/30 dark:border-platinum-700/30">
                <div>
                  <p className="text-sm font-semibold text-navy-500-900 dark:text-platinum-500-100">{src.sourceType}</p>
                  <p className="text-xs text-platinum-500-500">{src.count}+ sources</p>
                </div>
                <Badge

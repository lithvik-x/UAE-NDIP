...
export default function ControversyDebatePage() {
  ...
  const getTierColor = (tier: number) => {
    switch (tier) {
      ...
      case 1: return 'text-blue-400 bg-blue-500/20'
  ...
  const getTierBorder = (tier: number) => {
    switch (tier) {
      ...
      case 2: return 'border-yellow-500/50'
  ...
  const getSentimentColor = (score: number) => {
    ...
    if (score >= -0.2) return 'text-yellow-400 bg-yellow-500/20'
                  ...
                  .map((topic, idx) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <h4 className="font-semibold text-slate-200 text-sm">{topic.title}</h4>
                        <Badge className={getAlertColor(topic.alertLevel)} variant="outline">
                          {topic.alertLevel}
                        </Badge>
                      </div>
                      <p className="mt-2 text-xs text-slate-400 line-clamp-2">{topic.description.substring(0, 100)}...</p>
                      <div className="mt-3 flex items-center justify-between">
                        <span className="text-xs text-red-400">Score: {topic.sentimentScore.toFixed(2)}</span>
                        <span className="text-xs text-slate-500">Vol: {(topic.metrics.volume / 1000).toFixed(0)}K</span>
                      </div>
                    </motion.div>
                  ))}
              </div>
            ...
            {filteredTopics.map((topic, idx) => (
              ...
              >
                ...
                <CardContent>
                  ...
                  {/* Key Drivers */}
                  <div className="mt-4">
                    <h5 className="text-xs font-semibold text-slate-300 mb-2">Key Drivers</h5>
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                    <div className="p-2 rounded bg-platinum-500/10">
                      <div className="text-lg font-bold text-slate-200">{(topic.metrics.volume / 1000).toFixed(0)}K</div>
                      <div className="text-xs text-slate-500">Volume</div>
                    </div>
                    <div className="p-2 rounded bg-platinum-500/10">
                      <div className="text-lg font-bold text-slate-200">{(topic.metrics.reach / 1000000).toFixed(1)}M</div>
                      <div className="text-xs text-slate-500">Reach</div>
                    </div>
                    <div className="p-2 rounded bg-platinum-500/10">
                      <div className="text-lg font-bold text-slate-200">{topic.metrics.credibility}%</div>
                ...
                {controversySourceCredibilityMatrix.map((source, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium text-slate-200">{source.source}</div>

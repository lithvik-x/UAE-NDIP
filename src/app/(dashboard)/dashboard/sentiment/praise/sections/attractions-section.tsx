          ...
          {attractions.slice(0, 6).map((attraction, idx) => (
            ...
            >
              <CardContent className="p-4">
                ...
                <h4 className="font-semibold text-slate-200 mb-1">{attraction.entityName}</h4>
                <p className="text-xs text-slate-400">{attraction.category}</p>
              ...
              {visitorThemes.map((theme, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-200 text-sm italic">"{theme.theme}"</p>
                      <p className="text-xs text-slate-400">{theme.sourceType}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-400 mt-1">{theme.frequency}</p>
                  </div>
                </div>
              ))}
            </div>
              ...
              {attractions.map((attraction, idx) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-200 truncate">{attraction.entityName}</span>
                  </div>
                  <span className="text-amber-400 font-bold text-sm">{attraction.sentiment}</span>
                </div>
              ))}
              ...
              {sources.map((source, idx) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-400">{source.evidenceQuality}</p>
                  </div>
                  <Badge

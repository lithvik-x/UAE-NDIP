      ...
      >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
          <TabsList className="glass-panel flex-wrap h-auto gap-2">
            ...
            <TabsTrigger value="medium" className="data-[state=active]:bg-yellow-500/20">Medium ({negativeCategoriesEnriched.filter(c => c.severity === 'MEDIUM').length})</TabsTrigger>
                    ...
                    {isExpanded && (
                      ...
                      >
                        ...
                        {category.primaryConcerns.length > 0 && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Primary Concerns:</p>
                        ...
                        {category.keyQuotes && category.keyQuotes.length > 0 && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Key Quotes:</p>
                            <div className="space-y-2">
                              {category.keyQuotes.slice(0, 2).map((quote, i) => (
                                <blockquote key={i} className="text-xs text-slate-400 italic border-l-2 border-slate-600 pl-2">
                        ...
                        {'yemenWarData' in category && category.yemenWarData && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Yemen War Data:</p>
                        ...
                        {'cpiData' in category && category.cpiData && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Transparency International CPI (2024):</p>
                            <div className="bg-navy-900/50 rounded-lg p-3 space-y-1">
                        ...
                        {'arrestCases' in category && category.arrestCases && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Documented Arrest Cases:</p>
                            <div className="space-y-2">
                              {category.arrestCases.slice(0, 3).map((arrest, i) => (
                                <div key={i} className="bg-navy-900/50 rounded-lg p-2">
                                  <div className="flex justify-between">
                                    <span className="text-xs text-slate-300">{arrest.person}</span>
                                    <Badge variant="outline" className="text-xs">{arrest.nationality}</Badge>
                                  </div>
                                  <p className="text-xs text-slate-500 mt-1">{arrest.reason}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {'saadiyatIslandStats' in category && category.saadiyatIslandStats && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Saadiyat Island Statistics (HRW):</p>
                            <div className="bg-navy-900/50 rounded-lg p-3 space-y-1">
                        ...
                        {'iqAirData' in category && category.iqAirData && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">IQAir Data (August 2025):</p>
                            <div className="bg-navy-900/50 rounded-lg p-3 space-y-1">
                        ...
                        {'walkFreeData' in category && category.walkFreeData && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Walk Free Modern Slavery Index (2021):</p>
                            <div className="bg-navy-900/50 rounded-lg p-3 space-y-1">
                        ...
                        {category.dataPoints.length > 0 && (
                          <div>
                            <p className="text-xs text-slate-500 mb-2">Data Points:</p>

      ...
      <GlassCard hover className="h-full">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            ...
            <p className="text-sm text-gold font-medium mt-1">{official.title}</p>
          </div>
            ...
            <ul className="space-y-0.5">
              {official.achievements.slice(0, 3).map((ach, i) => (
                <li key={i} className="flex items-start gap-1.5 text-xs text-platinum-600 dark:text-platinum-400">
                  ...
                  {ach.year && <span className="text-gold font-medium">{ach.year}:</span>} {ach.achievement}
                </li>
              ))}
            </ul>
          </div>
    ...
    <div className="space-y-8 p-8">
      ...
      <Tabs defaultValue="supreme" className="space-y-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {data.supremeCouncilMembers.find(o => o.id === 'mbz')?.foreignPolicy?.map((fp, i) => (
                <div key={i} className="flex items-start gap-2 text-sm">
                  <Globe2 className="h-4 w-4 mt-0.5 text-gold shrink-0" />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {data.supremeCouncilMembers.find(o => o.id === 'mbr')?.achievements?.map((ach, i) => (
                <div key={i} className="flex items-start gap-2">
                  <Rocket className="h-4 w-4 mt-0.5 text-gold shrink-0" />
                  <div>
                    <span className="text-sm font-semibold text-gold">{ach.year}</span>
            ...
            <div className="space-y-3">
              ...
              <div className="pt-3 border-t border-platinum-200/50 dark:border-platinum-700/50">
                <p className="text-sm text-platinum-600 dark:text-platinum-400">
                  <span className="font-semibold text-gold">13 Global Clubs</span> — City Football Group, founded 2014, acquired Manchester City September 2008
                </p>
              </div>
            </div>
          </GlassPanel>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                ...
                <div key={i} className="glass-subtle rounded-lg p-3">
                  ...
                  <p className="text-xs text-gold">{s.handle}</p>
                ...
                <div className="grid gap-2 sm:grid-cols-2 mt-2">
                  {(cabinetData["deputyPrime Ministers"] ?? []).map((dpm, i) => (
                    <div key={i} className="text-sm">
                      ...
                      {dpm.additional && <p className="text-xs text-gold">{dpm.additional}</p>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              {(cabinetData.cabinetAffairsHighlights?.timeline ?? []).map((t, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div className="w-20 shrink-0">
                    <span className="text-xs font-bold text-gold">{t.year}</span>
                  </div>
        ...
        <TabsContent value="ministers" className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <GlassCard hover>
              ...
              <p className="text-xs text-gold mt-1">{data.notableMinisters.omarAlOlama.position}</p>
            ...
            <GlassCard hover>
              ...
              <p className="text-xs text-gold mt-1">{data.notableMinisters.shammaAlMazrui.positionHistory}</p>
              <div className="mt-3 space-y-1.5 text-xs">
                <p className="text-platinum-600 dark:text-platinum-400">
            ...
            <GlassCard hover>
              ...
              <p className="text-xs text-gold mt-1">{data.notableMinisters.sultanAlJaber.position}</p>
              <div className="mt-3 space-y-1.5 text-xs">
                  ...
                  <div className="pt-3 border-t border-platinum-200/50 dark:border-platinum-700/50">
                    ...
                    {m.initiatives.map((init, l) => (
                      <div key={l} className="flex items-start gap-2 mb-1">
                        ...
                        <div>
                          <span className="text-xs font-semibold text-gold">{init.name}:</span>
        ...
        <TabsContent value="initiatives" className="space-y-6">
          {/* Agentic AI */}
          <GlassCard className="border-2 border-gold/30">
                ...
                <div className="mt-3 space-y-2">
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div className="text-xs">
                      <span className="font-semibold text-gold">Authority:</span>
                    ...
                    <div className="text-xs">
                      <span className="font-semibold text-gold">Taskforce Chair:</span>
                      <span className="text-platinum-600 dark:text-platinum-400 ml-1">{data.recentInitiatives[0].taskforceChair}</span>
                    </div>
                    <div className="text-xs">
                      <span className="font-semibold text-gold">Oversight:</span>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {(recentInitiatives ?? []).slice(1).map((init, i) => (
              <GlassCard key={i} hover>
                ...
                <p className="text-sm text-platinum-700 dark:text-platinum-300">
                  {init.location && <span className="text-gold">{init.location}</span>}
                  {init.details}
                </p>
                {init.keyDiscussions && (
              ...
              <GlassCard key={i} hover>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gold mt-0.5">{proj.date || ''}</p>
        ...
        <TabsContent value="foreign" className="space-y-6">
          {/* Abraham Accords */}
          <GlassCard className="border-2 border-emerald/20">
            <div className="flex items-start gap-3">
              ...
              <Table>
                ...
                <TableBody>
                  {(foreignPolicyData.diplomaticVisitsMBZ ?? []).map((v, i) => (
                    <TableRow key={i}>
                      <TableCell className="text-xs text-gold font-medium">{v.date}</TableCell>
            ...
            <div className="space-y-2">
              {(foreignPolicyData.diplomaticVisitsMBR ?? []).map((v, i) => (
                <div key={i} className="flex items-start gap-3 text-sm">
                  <span className="text-xs text-gold font-medium w-20 shrink-0">{v.date}</span>
          ...
          <GlassPanel title="India-UAE Relations" badge="BILATERAL">
            <div className="space-y-2">
              {(foreignPolicyData.indiaUAERelations ?? []).map((r, i) => (
                <div key={i} className="flex items-start gap-3 text-sm">
                  <span className="text-xs text-gold font-medium w-16 shrink-0">{r.year}</span>
              ...
              {data.urlRepository.filter(u => u.tier === 'Official MBZ site' || u.type?.includes('Official') || u.type === 'UAE Government Portal' || u.type === 'UAE Cabinet official' || u.type === 'Ministry of Foreign Affairs' || u.type === 'Ministry of Finance' || u.type === 'Ministry of Interior' || u.type === 'Presidential Diwan' || u.type === 'MBR official site' || u.type === 'Maktoum site' || u.type === 'Hamdan bin Mohammed site' || u.type === 'Hamdan official site' || u.type === 'Fazza official site').map((u, i) => (
                <a key={i} href={u.url} target="_blank" rel="noopener noreferrer" className="block p-3 glass-subtle rounded-lg hover:shadow-glass transition-all">
                  <div className="flex items-center gap-2 mb-1">
                    <ExternalLink className="h-3 w-3 text-gold shrink-0" />

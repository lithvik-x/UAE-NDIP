...
import { mediaOrganizationsData } from '@/lib/data-loader'
import {
  Building2,
  Globe,
  TrendingUp,
...
export default function MediaOrganizationsPage() {
  const entities = mediaOrganizationsData
  ...
  // Tier distribution
  const tierDistribution = entities.reduce(
    (acc, e) => {
      acc[`Tier ${e.tier}`] = (acc[`Tier ${e.tier}`] || 0) + 1
      return acc
    },
    {} as Record<string, number>
  )
    ...
    <div className="space-y-8 p-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-3xl font-extrabold gradient-text-gold">Media Organizations</h1>
          <p className="mt-2 text-platinum-600 dark:text-platinum-400">
            Comprehensive intelligence on media organizations operating in the UAE and Gulf region.
            Track credibility, reach, sentiment, and relationships.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          ...
          description="Tracked media entities"
        />
        <MetricCard
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <GlassPanel title="Sentiment Breakdown" description="Overall sentiment distribution across media organizations">
        ...
        <GlassPanel title="Relevance Distribution" description="Top 10 organizations by UAE relevance">
          <div className="h-64">
            <BarChart
              data={relevanceData}
              xAxisKey="name"
              bars={[{ dataKey: 'relevance', name: 'Relevance', color: '#d4a574' }]}
              height={300}
              showGrid={true}
            />
          </div>
      ...
      {/* Alert Status */}
      <GlassPanel title="Alert Status Overview" description="Current alert levels across all media organizations">
      ...
      {/* Entity Table */}
      <GlassPanel title="Organization Directory" description="Detailed profiles of all tracked media organizations">
          ...
          <TableBody>
            {entities.map((entity) => (
              <TableRow key={entity.id}>
                ...
                <TableCell>
                  <Badge variant="outline" size="sm">{entity.role || 'Media'}</Badge>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-platinum-700 dark:text-platinum-300">
                      {entity.credibility?.reliabilityScore || 0}
                    </span>
                    {entity.credibility?.verified && (
      ...
      {/* Key Findings */}
      <GlassPanel title="Key Findings" description="Critical insights from media organization analysis">

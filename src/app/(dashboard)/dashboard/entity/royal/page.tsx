...
import { royalFamilyData } from '@/lib/data-loader'

export default function RoyalFamilyPage() {
  const data = royalFamilyData
  ...
  // Family breakdown
  const familyCounts = data.reduce((acc, e) => {
    const family = e.emirate || 'Other'
    acc[family] = (acc[family] || 0) + 1
    return acc
  }, {} as Record<string, number>)

  const familyData = Object.entries(familyCounts).map(([family, count]) => ({
    name: family,
    value: count,
    color: family === 'Abu Dhabi' ? '#d4af37' : family === 'Dubai' ? '#1e3a5f' : '#6b7280',
  }))
  ...
  return (
    <div className="space-y-8 p-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <Badge variant="navy" className="mb-2">ROYAL FAMILY</Badge>
          ...
          <p className="mt-2 text-platinum-600 dark:text-platinum-400">
            Royal family members — Al Nahyan & Al Maktoum dynasties — {totalCount} entities tracked
          </p>
        </div>
      ...
      <Tabs defaultValue="table" className="space-y-6">
        <TabsList className="glass-panel" scrollable>
          ...
          <TabsTrigger value="family">Family Breakdown</TabsTrigger>
          <TabsTrigger value="findings">Key Findings</TabsTrigger>
        </TabsList>

        {/* Entity Table */}
        <TabsContent value="table" className="space-y-6">
          <GlassPanel title="Royal Family Registry" description="Members of Al Nahyan and Al Maktoum families">
            <Table>
              <TableHeader>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card className="glass-card">
              <CardHeader>
                ...
                <CardDescription>Distribution of sentiment across royal family members</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    return (
                      <div key={tier} className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-platinum-500">{tierEntities.length} members · {pct}% positive</span>
                        </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card className="glass-card">
              <CardHeader>
                ...
                <CardDescription>Number of royal members per tier level</CardDescription>
              </CardHeader>
              <CardContent>
                <BarChart
                  data={tierData}
            ...
            <Card className="glass-card">
              <CardHeader>
                ...
                <CardDescription>Royal members with highest UAE relevance</CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {[...data]
                    ...
                    .map((entity, i) => (
                      <div key={entity.id} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-bold text-navy-600 dark:text-platinum-200">{entity.uaeRelevance}</span>
        ...
        <TabsContent value="family" className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-2">
            <Card className="glass-card">
              <CardHeader>
                <CardTitle className="text-lg">Family Distribution</CardTitle>
                <CardDescription>Breakdown by Al Nahyan and Al Maktoum families</CardDescription>
              </CardHeader>
              <CardContent>
                {familyData.length > 0 ? (
                  <PieChart
                  ...
                  <div className="flex items-center justify-center h-48 text-platinum-500">No family data available</div>
                )}
              </CardContent>
            </Card>
            <Card className="glass-card">
              <CardHeader>
                ...
                <CardDescription>List by family dynasty</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {familyData.map((family) => {
                    const members = data.filter(e => (e.emirate || 'Other') === family.name)
                    return (
                      <div key={family.name} className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          <span className="font-semibold text-platinum-700 dark:text-platinum-300">{family.name}</span>
                          <Badge variant="outline" className="text-xs">{family.value} members</Badge>
                        </div>
                        <div className="pl-2 space-y-1">
                          {members.slice(0, 4).map((m) => (
                            <div key={m.id} className="flex items-center justify-between text-xs text-platinum-600 dark:text-platinum-400">
                              <span>{m.name}</span>
                              <span className="text-platinum-400">Tier {m.tier}</span>
                            </div>
                          ))}
                          {members.length > 4 && (
                            <p className="text-xs text-platinum-500 pl-2">+{members.length - 4} more</p>
                          )}
                        </div>
                      </div>
                    )
                    ...
                    <ul className="space-y-1.5">
                      {entity.keyFindings.slice(0, 3).map((finding, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs text-platinum-700 dark:text-platinum-300">
                          <ChevronRight className="h-3 w-3 mt-0.5 text-navy-600 dark:text-platinum-400 shrink-0" />
                          {finding}
                        </li>
                      ))}
                    </ul>

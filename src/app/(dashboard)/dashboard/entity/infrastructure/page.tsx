...
import { infrastructureEntitiesData } from '@/lib/data-loader'
import type { EntityProfile } from '@/lib/data-loader/types'

export default function InfrastructurePage() {
  const entities = infrastructureEntitiesData
  ...
  const getInfrastructureIcon = (name: string) => {
    ...
    if (lowerName.includes('airport') || lowerName.includes('airline')) return <Plane className="h-4 w-4" />
    if (lowerName.includes('port') || lowerName.includes('harbor') || lowerName.includes('maritime')) return <Ship className="h-4 w-4" />
    if (lowerName.includes('rail') || lowerName.includes('metro') || lowerName.includes('train')) return <Train className="h-4 w-4" />
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          <Badge variant="emerald" className="text-sm font-bold tracking-wider">
            INFRASTRUCTURE
          </Badge>
        </div>
        <div>
          ...
          <p className="mt-1 text-platinum-600 dark:text-platinum-400">
            Critical infrastructure entities across air, sea, and land networks
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={entities.filter(e => e.tier <= 1).length}
        ...
        <Card className="glass-premium">
          ...
          <CardContent>
            <BarChart
              ...
              bars={[{ dataKey: 'count', name: 'Assets', color: '#10b981' }]}
              xAxisKey="tier"
              height={200}
              showLegend={false}
              showTooltip
      ...
      {/* Entity Table */}
      <GlassPanel title="Infrastructure Directory" description={`${totalCount} infrastructure assets tracked`}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/30">
                        {getInfrastructureIcon(entity.name)}
                      </div>
                      <div>
      ...
      {keyFindings.length > 0 && (
        <GlassPanel title="Key Findings" description="Critical infrastructure intelligence">

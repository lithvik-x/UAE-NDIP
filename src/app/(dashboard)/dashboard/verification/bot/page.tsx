...
export default function BotDashboardPage() {
  ...
  const getTierBadge = (tier: TierLevel) => {
    switch (tier) {
      ...
      case 2: return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50">Tier 2</Badge>
  ...
  const getUrlStatusBadge = (status: string) => {
    ...
    if (status.includes('PARTIAL')) return <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/50 text-xs">PARTIAL</Badge>
    ...
    if (status.includes('ACCESSED')) return <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/50 text-xs">ACCESSED</Badge>
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-1">
                          <span className="text-xs text-slate-500">Severity: <span className={getSeverityColor(item.severity)}>{item.severity}</span></span>
                          <span className="text-xs text-slate-500">Confidence: <span className="text-teal-400">{item.confidence}</span></span>
                        </div>
                      </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {data.dashboardKPIs.botNetworkScale.map((kpi: { kpi: string; value: string; lastUpdated: string }, idx: number) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-500 mt-1">{kpi.lastUpdated}</p>
                  </motion.div>
                ))}
              </div>
            </GlassPanel>
              ...
              <div className="space-y-6">
                <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
                  ...
                  <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-4 text-center">
                    ...
                    <p className="text-sm text-slate-400">Bot Percentage</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.rsfBotsData.tweetsViaTwitterWebApp}</div>
                    <p className="text-sm text-slate-400">Via Twitter Web App</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.rsfBotsData.accountsCreated5Month}</div>
                    <p className="text-sm text-slate-400">Created in 5 Months</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.rsfBotsData.postingBursts}</div>
                    <p className="text-sm text-slate-400">Posts per Burst</p>
                  </div>
                </div>
                ...
                {/* Narratives */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Narratives Promoted</h4>
                  <div className="space-y-2">
                    {data.rsfBotsData.narrativesPromoted.map((n: string, idx: number) => (
                      <motion.div
                        key={idx}
                        variants={fadeInUp}
                        className="flex items-start gap-2 rounded-lg border border-slate-700 bg-slate-800/50 px-3 py-2"
                      >
                ...
                {/* Detection Tools */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Detection Tools Used</h4>
                  <div className="flex flex-wrap gap-2">
              ...
              <div className="space-y-6">
                <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-5">
                  <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-4 text-center">
                    ...
                    <p className="text-sm text-slate-400">Bots Targeting COP28</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                  ...
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.cop28BotsData.generalAccountsOriginalContent}</div>
                    <p className="text-sm text-slate-400">General Accounts</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.cop28BotsData.minionAccountsAmplification}</div>
                    <p className="text-sm text-slate-400">Minion Accounts</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-orange-400">{data.cop28BotsData.maximumDailyQueriesBotometerLite}</div>
                    <p className="text-sm text-slate-400">Max Daily Queries</p>
                  </div>
                </div>

                {/* Network Structure */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Network Structure</h4>
                  <div className="space-y-2">
                    <div className="flex items-start gap-2 rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                      <Network className="h-4 w-4 text-emerald-400 mt-0.5" />
                      <p className="text-sm text-slate-300">{data.cop28BotsData.networkStructure.hierarchy}</p>
                    </div>
                    <div className="flex items-start gap-2 rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                      <Shield className="h-4 w-4 text-orange-400 mt-0.5" />
                      <p className="text-sm text-slate-300">{data.cop28BotsData.networkStructure.cells}</p>
                    </div>
                    <div className="flex items-start gap-2 rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                      <Clock className="h-4 w-4 text-teal-400 mt-0.5" />
                      <p className="text-sm text-slate-300">{data.cop28BotsData.networkStructure.accountRecycling}</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Content Focus */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Content Focus</h4>
                  <div className="space-y-2">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Verified General Accounts</h4>
                  <div className="flex flex-wrap gap-2">
                ...
                <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4">
                  ...
                  <p className="text-sm text-slate-400">Researcher: {data.cop28BotsData.methodology.researcher}</p>
                  <p className="text-sm text-slate-400">Tools: {data.cop28BotsData.methodology.tools}</p>
                </div>
              </div>
            </GlassPanel>
          </motion.div>
              ...
              <div className="space-y-6">
                <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
                  <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-4 text-center">
                    ...
                    <p className="text-sm text-slate-400">Bot Accounts Detected</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-emerald-400">{data.stcBotsData.estimatedActualBots}</div>
                    <p className="text-sm text-slate-400">Estimated Actual Bots</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.stcBotsData.tweetsAnalyzed}</div>
                    <p className="text-sm text-slate-400">Tweets Analyzed</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.stcBotsData.uniqueAccountsInDataset}</div>
                    <p className="text-sm text-slate-400">Unique Accounts</p>
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-4">
                  ...
                  <p className="text-sm text-slate-300">{data.stcBotsData.keyFinding}</p>
                </div>
              </div>
            </GlassPanel>
          </motion.div>
              ...
              <div className="space-y-6">
                <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
                  <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-4 text-center">
                    ...
                    <p className="text-sm text-slate-400">Troll Army Size</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-emerald-400">{data.trollFarmData.accountsPerTroll}</div>
                    <p className="text-sm text-slate-400">Accounts per Troll</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-gold">{data.trollFarmData.monthlyPaymentSeniorTroll}</div>
                    <p className="text-sm text-slate-400">Senior Troll Pay</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.trollFarmData.monthlyPaymentEntryLevel}</div>
                    <p className="text-sm text-slate-400">Entry-Level Pay</p>
                  </div>
                </div>

                {/* Operational Structure */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Operational Structure</h4>
                  <div className="space-y-2">
              ...
              <div className="space-y-6">
                <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
                  ...
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    ...
                    <p className="text-sm text-slate-400">Tweets/Day</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.diavoloNetworkData.accountsSuspended2016}</div>
                    <p className="text-sm text-slate-400">Suspended 2016</p>
                  </div>
                  <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4 text-center">
                    <div className="text-2xl font-bold text-slate-200">{data.diavoloNetworkData.monthlyAccountCreationAvg}</div>
                    <p className="text-sm text-slate-400">Monthly Creation Avg</p>
                  </div>
                </div>

                {/* Hashtag Distribution */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Hashtag Distribution</h4>
                  <div className="space-y-2">
                ...
                {/* Network Details */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Network Details</h4>
                  <div className="space-y-2">
                ...
                {data.globalTrollFarmData.globalTrollFarmOperations.map((op: { country: string; operation: string; details: string }, idx: number) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-slate-200">{op.country}</p>
                        <p className="text-sm text-slate-400">{op.operation}</p>
                      </div>
                    </div>
          ...
          <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-6">
            ...
            <GlassPanel title="Botometer Technical Specifications" description="Bot detection tool capabilities">
              <div className="space-y-6">
                <div className="grid gap-4 lg:grid-cols-2">
                  <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between"><span className="text-slate-400">Daily Queries:</span><span className="text-slate-200">{data.botometerSpecs.features[0].dailyQueries}</span></div>
                      <div className="flex justify-between"><span className="text-slate-400">Data Required:</span><span className="text-slate-200">{data.botometerSpecs.features[0].dataRequired}</span></div>
                      <div className="flex justify-between"><span className="text-slate-400">Speed:</span><span className="text-slate-200">{data.botometerSpecs.features[0].speed}</span></div>
                    </div>
                  </div>
                  <div className="rounded-lg border border-denim-500/30 bg-denim-500/10 p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between"><span className="text-slate-400">Daily Queries:</span><span className="text-slate-200">{data.botometerSpecs.features[1].dailyQueries}</span></div>
                      <div className="flex justify-between"><span className="text-slate-400">Data Required:</span><span className="text-slate-200">{data.botometerSpecs.features[1].dataRequired}</span></div>
                      <div className="flex justify-between"><span className="text-slate-400">Speed:</span><span className="text-slate-200">{data.botometerSpecs.features[1].speed}</span></div>
                    </div>
                  </div>
                </div>
                ...
                {/* Accuracy Metrics */}
                <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4">
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Accuracy Metrics</h4>
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-emerald-400">{data.botometerSpecs.accuracyMetrics.crossValidationAUC}</div>
                      <p className="text-xs text-slate-400">Cross-Validation AUC</p>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-slate-200">0-5</div>
                      <p className="text-xs text-slate-400">Display Scale</p>
                    </div>
                  </div>
                </div>
                ...
                {/* Features Analyzed */}
                <div>
                  <h4 className="text-sm font-semibold text-slate-300 mb-3">Features Analyzed</h4>
                  <div className="flex flex-wrap gap-2">
                    {data.botometerSpecs.featuresAnalyzed.map((f: string, idx: number) => (
                      <Badge key={idx} variant="outline" className="bg-slate-700/50">{f}</Badge>
                    ))}
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {data.botDetectionIndicators.visualProfile.map((ind: { indicator: string; description: string; detectionWeight: string }, idx: number) => (
                      <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/50 px-3 py-2">
                        <div>
                          <p className="text-sm text-slate-200">{ind.indicator}</p>
                          <p className="text-xs text-slate-500">{ind.description}</p>
                        </div>
                        <Badge variant="outline" className={ind.detectionWeight === 'High' ? 'border-rose-500/50 text-rose-400' : 'border-slate-600 text-slate-400'}>{ind.detectionWeight}</Badge>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-2">
                    {data.botDetectionIndicators.behavioral.map((ind: { indicator: string; statistic: string; source: string }, idx: number) => (
                      <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/50 px-3 py-2">
                        <div>
                          <p className="text-sm text-slate-200">{ind.indicator}</p>
                          <p className="text-xs text-slate-500">Source: {ind.source}</p>
                        </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {data.dashboardKPIs.engagementFraud.map((kpi: { kpi: string; value: string; source: string }, idx: number) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-400 mt-1">{kpi.kpi}</p>
                    <p className="text-xs text-slate-500 mt-1">Source: {kpi.source}</p>
                  </motion.div>
                ))}
              </div>
            </GlassPanel>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {data.dashboardKPIs.impact.map((kpi: { kpi: string; value: string; context: string }, idx: number) => (
                  ...
                  >
                    <div className="text-2xl font-bold text-emerald-400">{kpi.value}</div>
                    <p className="text-sm text-slate-400 mt-1">{kpi.kpi}</p>
                    <p className="text-xs text-slate-500 mt-1">{kpi.context}</p>
                  </motion.div>
                ))}
              </div>
            </GlassPanel>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-400">{entity.role}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <Badge variant="outline" className="text-xs">{entity.type}</Badge>
                        {entity.country && <p className="text-xs text-slate-500 mt-1">{entity.country}</p>}
                      </div>
                    </motion.div>
                  ))}
                </div>
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-xs text-slate-500">Score: {source.credibilityScore}/10</span>
                          <span className="text-xs text-slate-500">{source.trustLevel}</span>
                        </div>
                      </div>
            ...
            <GlassPanel title="URL Fetch Status Log" description="Research source accessibility breakdown">
              <div className="grid gap-4 lg:grid-cols-3 mb-6">
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4 text-center">
                  ...
                  <p className="text-sm text-slate-400">Fully Accessible</p>
                  <p className="text-xs text-slate-500">{data.fetchSummary.fullyAccessiblePercent}</p>
                </div>
                <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-4 text-center">
                  <div className="text-3xl font-bold text-yellow-400">{data.fetchSummary.partiallyAccessible}</div>
                  <p className="text-sm text-slate-400">Partially Accessible</p>
                  <p className="text-xs text-slate-500">{data.fetchSummary.partiallyAccessiblePercent}</p>
                </div>
                <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-center">
                  <div className="text-3xl font-bold text-red-400">{data.fetchSummary.blocked}</div>
                  <p className="text-sm text-slate-400">Blocked</p>
                  <p className="text-xs text-slate-500">{data.fetchSummary.blockedPercent}</p>
                </div>
              </div>
                  ...
                  {data.urlFetchStatusLog.map((url: { url: string; status: string; dataExtracted?: string; contentQuality: string }, idx: number) => (
                    <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/50 px-3 py-2">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-slate-300 truncate">{url.url}</p>
                        {url.dataExtracted && <p className="text-xs text-slate-500">Extracted: {url.dataExtracted}</p>}
                      </div>
                ...
                <div className="space-y-3">
                  {data.atomicQueriesExecuted.map((q: { number: number; query: string; status: string; keyFindings: string }, idx: number) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-slate-200 text-sm">{q.query}</p>
                        <p className="text-xs text-slate-500 mt-1">Findings: {q.keyFindings}</p>
                      </div>
                ...
                <div className="space-y-3">
                  {data.sentimentAnalysisByTopic.map((item: { topic: string; overallSentiment: string; dominantNarrative: string; sourceDiversity: string }, idx: number) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-slate-400 mb-1">{item.dominantNarrative}</p>
                      <p className="text-xs text-slate-500">Sources: {item.sourceDiversity}</p>
                    </motion.div>
                  ))}
                </div>
              </ScrollArea>

            ...
            {consensusMatrix.map((item, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <p className="font-medium text-slate-200">{item.finding}</p>
                  <Badge className={
                    item.consensusLevel === 'HIGH' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' :
                    item.consensusLevel.includes('MODERATE') ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50' :
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <span className="text-slate-500">Supporting: {item.supportingSources}</span>
                  {item.challengingSources && (
                    <span className="text-slate-500">Challenging: {item.challengingSources}</span>
                  )}
                </div>
              </motion.div>
            ))}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-semibold text-slate-200">{method.method}</h4>
                </div>
              ...
              <p className="text-xs text-slate-500">Limitation: {method.keyLimitation}</p>
            </motion.div>
          ))}
        </div>
      </GlassPanel>
            ...
            {quantitativeMetrics.map((metric, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-mono text-slate-500">{metric.id}</span>
                  <span className="text-xs text-slate-500">{metric.year}</span>
                </div>
                <p className="text-xs text-slate-300 mb-1 truncate">{metric.metric}</p>
            ...
            {qualitativeFindings.map((finding, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-200 mb-1">{finding.finding}</p>

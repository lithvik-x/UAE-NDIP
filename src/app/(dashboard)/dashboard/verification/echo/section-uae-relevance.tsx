      ...
      >
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">Score out of 5</p>
          </div>
        </div>
            ...
            {uaeRelevanceMatrix.map((item, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-200">{item.topicArea}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-sm text-slate-400 max-w-[300px]">{item.rationale}</span>
                  <Badge className={
          ...
          {uaeConsiderations.map((item, idx) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="font-semibold text-slate-200">{item.factor}</h4>
              </div>
          ...
          <div className="space-y-3">
            {regionalDynamics.map((dynamic, idx) => (
              <motion.div
                ...
                className="flex items-center justify-between rounded-lg border border-slate-700/50 bg-slate-800/30 p-3"
              >
                <span className="font-medium text-slate-200">{dynamic.dynamic}</span>
                <div className="flex items-center gap-3">
                  <Badge className={
                    dynamic.presentInUAERegion === 'YES' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/50' :
                    dynamic.presentInUAERegion === 'LIMITED' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50' :
                    'bg-rose-500/20 text-rose-400 border-rose-500/50'
                  }>
                    {dynamic.presentInUAERegion}
                  </Badge>
                  <span className="text-sm text-slate-400 max-w-[200px]">{dynamic.evidence}</span>
                </div>
              </motion.div>
            ))}
          </div>
            ...
            {threatAssessments.map((threat, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-200">{threat.threat}</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex gap-2">
                    <Badge variant="outline" className={
                      threat.probability === 'HIGH' ? 'text-rose-400 border-rose-500/50' :
                      threat.probability === 'MEDIUM' ? 'text-yellow-400 border-yellow-500/50' :
                      'text-slate-400'
                    }>
                    ...
                    <Badge variant="outline" className={
                      ...
                      threat.impact === 'MEDIUM' ? 'text-yellow-400 border-yellow-500/50' :
                      'text-slate-400'
                    }>
                      I: {threat.impact}
                    </Badge>
                  ...
                  <Badge className={
                    ...
                    threat.combinedScore === 'MEDIUM' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/50' :

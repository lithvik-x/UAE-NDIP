    ...
    <div className="space-y-8 p-8">
      ...
      >
        <div>
          ...
          <h1 className="text-3xl font-extrabold font-rajdhani gradient-text-platinum">Threads Intelligence</h1>
                    ...
                    {notableAccountsData.map((account, idx) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          {account.followers && (
                            <p className="text-xs text-gold mt-1">{account.followers.toLocaleString()} followers</p>
                          )}
                        </div>
                      </motion.div>
                    ))}
                    ...
                    {trends2026Data.map((trend, idx) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium text-slate-200">{trend.trend}</span>
                          <Badge variant={trend.impact === 'High' ? 'default' : 'outline'} className={trend.impact === 'High' ? 'bg-gold/20 text-gold' : ''}>
                            {trend.impact}
                          </Badge>
                        </div>
                        ...
                        <div key={idx} className="flex items-center justify-between rounded-lg bg-slate-800/30 p-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">{source.type} - {source.uaeRelevance}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                <CardContent>
                  <div className="space-y-3">
                    {monitoringRecsData.map((rec, idx) => (
                      <div key={idx} className="flex items-center justify-between rounded-lg bg-slate-800/50 p-3">
                        <span className="text-sm text-slate-200">{rec.metric}</span>

  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-platinum-400">Loading Maghrebi Arabic intelligence data...</div>
      </div>
    )
  }
    ...
    <div className="space-y-8 p-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-3xl font-extrabold gradient-text-platinum">Maghrebi Arabic Dialect</h1>
          <p className="mt-2 text-slate-400">
            {data.description || 'Intelligence on Maghrebi Arabic dialect: Morocco, Algeria, Tunisia, Libya'}
          </p>
        </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        <MetricCard
          title="Query Volume"
          value={data.queryVolume?.toLocaleString() || '12,400,000'}
          previousValue={(data.queryVolume || 12400000) - 300000}
        ...
        <MetricCard
          title="UAE Speakers"
          value={uaeSpeakerPopulation?.toLocaleString() || '150,000'}
          previousValue={(uaeSpeakerPopulation || 150000) + 10000}
          icon={<Users className="h-6 w-6" />}
          gradient="navy"
        />
        <MetricCard
          title="Sentiment Score"
          value={sentiment?.overall || 24}
          previousValue={(sentiment?.overall || 24) + 3}
          icon={<TrendingUp className="h-6 w-6" />}
          gradient="emerald"
        />
        <MetricCard
          title="Credibility Score"
          value={credibility?.score || 76}
          previousValue={(credibility?.score || 76) - 1}
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        <TabsList className="glass-panel" scrollable>
          ...
          <TabsTrigger value="regional">Regional Tensions</TabsTrigger>
        </TabsList>
            ...
            <div className="space-y-6">
              <Card className="glass-card">
                <CardHeader>
                  ...
                  <CardDescription>Maghrebi Arabic speaker distribution</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-4 lg:grid-cols-4">
                    <div className="text-center p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      <div className="text-2xl font-bold text-emerald-400">38M</div>
                      <p className="text-sm text-slate-400">Morocco</p>
                    </div>
                    <div className="text-center p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      <div className="text-2xl font-bold text-rose-400">46M</div>
                      <p className="text-sm text-slate-400">Algeria</p>
                    </div>
                    <div className="text-center p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      <div className="text-2xl font-bold text-gold-400">12M</div>
                      <p className="text-sm text-slate-400">Tunisia</p>
                    </div>
                    <div className="text-center p-4 rounded-lg bg-slate-800/50 border border-slate-700">
                      <div className="text-2xl font-bold text-slate-400">7M</div>
                      <p className="text-sm text-slate-400">Libya</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <Card className="glass-card">
                  <CardHeader>
                    <CardTitle className="text-lg">UAE Diaspora Profile</CardTitle>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                        <div className="flex items-center gap-3">
                          <MapPin className="h-5 w-5 text-platinum-400" />
                          <span className="text-slate-200">Moroccan Diaspora</span>
                        </div>
                        <Badge className="bg-emerald-500/20 text-emerald-400">Significant</Badge>
                      </div>
                      <div className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                        <div className="flex items-center gap-3">
                          <Users className="h-5 w-5 text-gold-400" />
                          <span className="text-slate-200">Algerian Community</span>
                        </div>
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-slate-800/50 border border-slate-700">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-slate-200">Tunisian Workers</span>
                        </div>
                        <Badge className="bg-emerald-500/20 text-emerald-400">Growing</Badge>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  <div className="grid gap-4 lg:grid-cols-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="text-2xl font-bold text-emerald-400">{credibility?.sources || 12}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-lg font-bold text-emerald-400">{sentiment?.positive || 52}%</span>
                      </div>
                      <Progress value={sentiment?.positive || 52} className="h-3" />
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-lg font-bold text-rose-400">{sentiment?.negative || 28}%</span>
                      </div>
                      <Progress value={sentiment?.negative || 28} className="h-3" />
                    </div>
                  </div>
                </CardContent>
              </Card>
                        ...
                        <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-700 bg-slate-800/50 p-4 hover:bg-slate-800/70">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-slate-400">{source.date}</p>
                            </div>
                          </div>
